    ...
    if (isset($_COOKIE['remember_user'])) {
        ...
        if ($conn) {
            try {
                ...
                $result = db_query($conn, $query, array($userId));
                
                if (db_num_rows($result) > 0) {
                    $user = db_fetch_assoc($result);
                    $_SESSION['user_id'] = $user['id'];
                    $_SESSION['username'] = $user['username'];
                    $_SESSION['display_name'] = $user['username']; // Using username as display_name
                }
            } catch (Exception $e) {
...
if ($conn) {
    try {
        $query = "SELECT * FROM assignments WHERE user_id = $1 ORDER BY due_date ASC";
        $result = db_query($conn, $query, array($_SESSION['user_id']));
        $assignments = db_fetch_all($result);
    } catch (Exception $e) {
        // Log the error
                ...
                <div class="assignment-filters">
                    <div class="filter-group">
                        ...
                        <select id="courseFilter" class="form-select">
                            ...
                            if ($conn) {
                                try {
                                    $query = "SELECT DISTINCT course FROM assignments WHERE user_id = $1 ORDER BY course";
                                    $result = db_query($conn, $query, array($_SESSION['user_id']));
